...
we need: training (60%), validation (20%) and test (20%). It takes account a percentage of GC on which to sample from the data. The data contains less SSPs then stars, so the loading process fixes the number of SSPs and samples a number of stars accordingly to keep the proportion. The default value is 0.1.


```{r}
load_dataset = function(vals, tests, sample_percentage = 0.1) {
...
tests = .2  # test split
ssp_perc = 0.08
samples = load_dataset(vals,tests, ssp_perc)  # loading dataset
